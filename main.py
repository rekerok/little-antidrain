...
from token import Token_Amount

CHECK_INTERVAL = 1  # как часто чекать кошель в секундах
...
async def create_web3_instance(rpcs):
    for rpc in rpcs:
        w3 = AsyncWeb3(
            AsyncHTTPProvider(
                endpoint_uri=rpc,
    ...
    while True:
        try:
            ...
            balance = Token_Amount(
                amount=await contract.functions.balanceOf(
...
async def main():
    ...
    logger.info("start")
    tasks = [check_balance(pair) for pair in pairs]
